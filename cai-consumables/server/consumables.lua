...
for k, v in pairs(Config.Consumables) do
	QBCore.Functions.CreateUseableItem(k, function(source, item) TriggerClientEvent('cai-consumables:Consume', source, item.name) end)
...
RegisterNetEvent("cai-consumables:server:DupeWarn", function(item, newsrc)
...
RegisterNetEvent('cai-consumables:server:toggleItem', function(give, item, amount, newsrc)
	local src = newsrc or source
	...
	if not give then
		...
		else TriggerEvent("cai-consumables:server:DupeWarn", item, src) end -- if not boot the player
	elseif give then
...
RegisterNetEvent('cai-consumables:server:addNeed', function(amount, type)
...
QBCore.Functions.CreateCallback('cai-consumables:server:syncConsumables', function(source, cb) cb(Consumables) end)
QBCore.Functions.CreateCallback('cai-consumables:server:syncEmotes', function(source, cb) cb(Emotes) end)

RegisterNetEvent('cai-consumables:server:syncAddItem', function(itemName, data)
	if not Consumables[itemName] then
		QBCore.Functions.CreateUseableItem(itemName, function(source, item) TriggerClientEvent('cai-consumables:Consume', source, itemName) end)
		if Config.Debug then print("^5Debug^7: ^2Adding new ^3Item^7: '^6"..itemName.."^7'") end
		Consumables[itemName] = data
		TriggerClientEvent("cai-consumables:client:syncConsumables", -1, Consumables)
	end
end)

RegisterNetEvent('cai-consumables:server:syncAddEmote', function(emoteName, data)
	if not Emotes[emoteName] then
		...
		TriggerClientEvent("cai-consumables:client:syncEmotes", -1, Emotes)
	end
end)

RegisterNetEvent("cai-consumables:server:syncConsumables", function() TriggerClientEvent('cai-consumables:client:syncConsumables', -1, Consumables) end)
RegisterNetEvent("cai-consumables:server:syncEmotes", function() TriggerClientEvent('cai-consumables:client:syncEmotes', -1, Emotes) end)

--local function CheckVersion()
--	PerformHttpRequest('https://raw.githubusercontent.com/jimathy/cai-consumables/master/version.txt', function(err, newestVersion, headers)
