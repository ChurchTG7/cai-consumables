...
local function syncConsumables()
	local p = promise.new() QBCore.Functions.TriggerCallback('cai-consumables:server:syncConsumables', function(cb) p:resolve(cb) end) Consumables = Citizen.Await(p)
	local p2 = promise.new() QBCore.Functions.TriggerCallback('cai-consumables:server:syncEmotes', function(cb) p2:resolve(cb) end) Emotes = Citizen.Await(p2)
    if Config.Debug then print("^5Debug^7: ^2Retrieved ^6"..countTable(Consumables).." ^2Items and ^6"..countTable(Emotes).." ^2Emotes^7") end
    propConvert()
end

RegisterNetEvent("cai-consumables:client:syncConsumables", function(NewConsumables)
...
RegisterNetEvent("cai-consumables:client:syncEmotes", function(NewEmotes)
...
RegisterNetEvent('cai-consumables:Consume', function(itemName)
	...
	if not cancelled then
        ...
        if stats then
            if stats.hunger then TriggerServerEvent("cai-consumables:server:addNeed", QBCore.Functions.GetPlayerData().metadata["hunger"] + stats.hunger, "hunger") end
            if stats.thirst then TriggerServerEvent("cai-consumables:server:addNeed", QBCore.Functions.GetPlayerData().metadata["thirst"] + stats.thirst, "thirst") end
        end
        ...
        if stats then
            ...
            if stats.effect == "heal" then
                if Config.Debug then print("^5Debug^7: ^3Consume^7: ^6Health Buff ^2for ^6"..stats.time.."^7ms") end
                CreateThread(function() HealEffect({(tonumber(stats.time) or 10000), (stats.amount or 6)}) end)
            end
            if stats.effect == "stamina" then
                if Config.Debug then print("^5Debug^7: ^3Consume^7: ^6Stamina Buff ^2for ^6"..stats.time.."^7ms") end
                CreateThread(function() StaminaEffect({(tonumber(stats.time) or 10000), (stats.amount or 6)}) end)
            end
            if stats.effect == "armor" then
                if Config.Debug then print("^5Debug^7: ^3Consume^7: ^6Armor Buff for ^6"..(stats.time or "nil").."^7ms") end
                CreateThread(function() AddArmorBuff({(tonumber(stats.time) or 10000), (stats.amount or 6)}) end)
            end
            if stats.effect == "stress" then
                if Config.Debug then print("^5Debug^7: ^3Consume^7: ^6Stress Buff for ^6"..(stats.time or "nil").."^7ms") end
                CreateThread(function() AddStressBuff({(tonumber(stats.time) or 10000), (stats.amount or 6)}) end)
            end
            if stats.effect == "swimming" then
                if Config.Debug then print("^5Debug^7: ^3Consume^7: ^6Swimming Buff for ^6"..(stats.time or "nil").."^7ms") end
                CreateThread(function() SwimmingBuffEffect({(tonumber(stats.time) or 10000), (stats.amount or 6)}) end)
            end
            if stats.effect == "hacking" then
                if Config.Debug then print("^5Debug^7: ^3Consume^7: ^6Hacking Buff for ^6"..(stats.time or "nil").."^7ms") end
                CreateThread(function() AddBuff("hacking", (tonumber(stats.time) or 10000)) end)
            end
            if stats.effect == "intelligence" then
                if Config.Debug then print("^5Debug^7: ^3Consume^7: ^6Intelligence Buff for ^6"..(stats.time or "nil").."^7ms") end
                CreateThread(function() AddBuff("intelligence", (tonumber(stats.time) or 10000)) end)
            end
            if stats.effect == "luck" then
                if Config.Debug then print("^5Debug^7: ^3Consume^7: ^6Luck Buff for ^6"..(stats.time or "nil").."^7ms") end
                CreateThread(function() AddBuff("luck", (tonumber(stats.time) or 10000)) end)
            end
            if stats.effect == "strength" then
                if Config.Debug then print("^5Debug^7: ^3Consume^7: ^6Strength Buff for ^6"..(stats.time or "nil").."^7ms") end
                CreateThread(function() AddBuff("strength", (tonumber(stats.time) or 10000)) end)
            end
